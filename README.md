...
> test 集（86 张），nIoU 为论文主指标 (Eq. 1)。下表为 **100 epoch** 复现结果
> （`run_all.py --preset full --epochs 100`）。跑满 300 epoch 数值会更高。

### 主结果

| 模型 | 配置 | IoU | nIoU |
|---|---|---|---|
| ACM-FPN | 论文 ACM | 0.6653 | 0.7002 |
| ACM-U-Net | 论文 ACM | 0.6406 | 0.6963 |
| ACM-FPN | **ACM++（改进后）** | **0.6712** | **0.7037** |

### 消融：调制方案（ACM-FPN host，对应论文 Table 2）

| 方案 | 说明 | IoU | nIoU |
|---|---|---|---|
| TopDownLocal | 单向 | 0.6672 | 0.6969 |
| BiLocal | 双向局部 | 0.6351 | 0.6806 |
| BiGlobal | 双向全局 | 0.6388 | 0.7052 |
| **ACM** | **非对称（本文）** | **0.6653** | **0.7002** |

### 消融：下采样方案（ACM-FPN host）

| 方案 | 总降采样 | IoU | nIoU |
|---|---|---|---|
| adjusted（论文） | /4 | **0.6653** | **0.7002** |
| regular（标准 ResNet） | /16 | 0.6374 | 0.6749 |

> 下采样消融印证论文核心论点：保住分辨率的 adjusted 方案（/4）显著优于标准 ResNet 的 /16，
> 说明**抑制降采样对小目标至关重要**。
